...
namespace ResourceTools
{
    ...
    public class UnloadBundleTask : BaseTask
    {
        ...
        public override float Progress
        {
            get
            {
                return timer / AssetBundlesManager.UnloadDelayTime;
            }
        }

        public UnloadBundleTask(TaskExcutor owner, string name) : base(owner, name)
        {
            bundleInfo = AssetBundlesManager.bundleInfoDict[name];
        ...
        public override void Update()
        {
            ...
            if (timer < AssetBundlesManager.UnloadDelayTime)
            {
            ...
            for (int i = 0; i < bundleInfo.ManifestInfo.Assets.Length; i++)
            {
                ...
                if (AssetBundlesManager.assetInfoDict.TryGetValue(assetName,out AssetRuntimeInfo assetInfo))
                {
                    if (assetInfo.Asset != null)
                    {
                        AssetBundlesManager.assetToAssetInfoDict.Remove(assetInfo.Asset);
            ...
            {
                BundleRuntimeInfo dependencyBundleInfo = AssetBundlesManager.bundleInfoDict[bundleName];
                dependencyBundleInfo.DependencyCount--;
                AssetBundlesManager.CheckBundleLifeCycle(dependencyBundleInfo);
            }
