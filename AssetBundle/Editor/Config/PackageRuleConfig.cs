...
namespace ResourceTools.Editor
{
    ...
    public class PackageRuleConfig : ScriptableObject
    {
        ...
        [MenuItem("ResourceTools/创建打包规则配置文件")]
        ...
        public  List<AssetBundleBuild> GetAssetBundleBuildList(bool isAnalyzeRedundancy = true)
        {
            ...
            foreach (PackageRule rule in Rules)
            {
                ...
                if (abBuilds != null)
                {
                    ...
                    if (rule.InPackage)
                    {
                        foreach (var bundle in abBuilds)
                        {
                            AssetCollector.AddLocalAssetBundleGroup(bundle.assetBundleName);
                        }
                    }
                }
            ...
            for (int i = 0; i < abBuildList.Count; i++)
            {
                ...
                if (SplitSceneBundleAsset(abBulid,out AssetBundleBuild sceneBundle,out AssetBundleBuild notSceneBundle))
                {
                    ...
                    if (AssetCollector.IsLocalAssetBundle(abBulid.assetBundleName))
                    {
                        AssetCollector.AddLocalAssetBundleGroup(sceneBundle.assetBundleName);
                        AssetCollector.AddLocalAssetBundleGroup(notSceneBundle.assetBundleName);
                    }
