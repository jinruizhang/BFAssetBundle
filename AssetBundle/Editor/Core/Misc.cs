...
namespace ResourceTools.Editor
{
    ...
    public static class Misc
    {
        ...
        [MenuItem("ResourceTools/打开目录/打包输出根目录", priority = 2)]
        private static void OpenAssetBundleOutputPath()
        {
            Open(Path.Combine(Directory.GetCurrentDirectory(),PkgUtil.PkgCfg.OutputPath));
        }

        [MenuItem("ResourceTools/打开目录/只读区", priority = 2)]
        private static void OpenReadOnlyPath()
        {
            Open(Application.streamingAssetsPath);
        }

        [MenuItem("ResourceTools/打开目录/读写区", priority = 2)]
